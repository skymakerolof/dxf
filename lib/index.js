...
function spline_toSVGPath(spline, transform) {
    ...
    if (controlcount > 3 && knotcount === controlcount + 4) { // Cubic Spline
        if (controlcount > 4) {
            for (var j = knotcount - 5; j > 3; j--) {
                if (knots[j] != knots[j - 1] && knots[j] != knots[j + 1]) {
                    var a0 = knots[j] - knots[j - 2] / (knots[j + 1] - knots[j - 2]);
                    var a1 = knots[j] - knots[j - 1] / (knots[j + 2] - knots[j - 1]);
                    point = {
...
Collector.prototype.getBBox = function Collector_getBBox(layers, depth) {
    ...
    for (var i = 0; i < lines.length; i++) {
        var line = lines[i];
        if (allLayers || layers.indexOf(line.layer) !== -1) {
            if (line.start.x < bbox.left) bbox.left = line.start.x;
            if (line.start.x > bbox.right) bbox.right = line.start.x;
            if (line.start.y < bbox.bottom) bbox.bottom = line.start.y;
            if (line.start.y > bbox.top) bbox.top = line.start.y;
            if (line.end.x < bbox.left) bbox.left = line.end.x;
            if (line.end.x > bbox.right) bbox.right = line.end.x;
            if (line.end.y < bbox.bottom) bbox.bottom = line.end.y;
            if (line.end.y > bbox.top) bbox.top = line.end.y;
        }
    }
    ...
    for (var i = 0; i < polylines.length; i++) {
        ...
        if (allLayers || layers.indexOf(polyline.layer) !== -1) {
            for (var j = 0; j < polyline.points.length; j++) {
                ...
                if (point.y < bbox.bottom) bbox.bottom = point.y;
    ...
    for (var i = 0; i < arcs.length; i++) {
        ...
        if (allLayers || layers.indexOf(arc.layer) !== -1) {
            ...
            if (arc.x < bbox.left) bbox.left = arc.x;
            if (arc.x > bbox.right) bbox.right = arc.x;
            if (arc.y < bbox.bottom) bbox.bottom = arc.y;
            if (arc.y > bbox.top) bbox.top = arc.y;
        }
    }
    ...
    for (var i = 0; i < texts.length; i++) {
        ...
        if (allLayers || layers.indexOf(texts.layer) !== -1) {
            ...
            if (text.x < bbox.left) bbox.left = text.x;
            if (text.x > bbox.right) bbox.right = text.x;
            if (text.y < bbox.bottom) bbox.bottom = text.y;
            if (text.y > bbox.top) bbox.top = text.y;
        }
    }
    ...
    for (var i in blocks) {
        if (i.charAt(0) !== '*' && blocks.hasOwnProperty(i) && allLayers || layers.indexOf(i) !== -1) {
            ...
            if (block_bbox.left < bbox.left) bbox.left = block_bbox.left;
            if (block_bbox.right > bbox.right) bbox.right = block_bbox.right;
            if (block_bbox.bottom < bbox.bottom) bbox.bottom = block_bbox.bottom;
            if (block_bbox.top > bbox.top) bbox.top = block_bbox.top;
        }
    }
    ...
    for (var i = 0; i < dimensions.length; i++) {
        ...
        if (allLayers || layers.indexOf(dimension.layer) !== -1) {
            if (dimension.block_name) {
                ...
                if (dimension.x < bbox.left) bbox.left = dimension.x;
                if (dimension.x > bbox.right) bbox.right = dimension.x;
                if (dimension.y < bbox.bottom) bbox.bottom = dimension.y;
                if (dimension.y > bbox.top) bbox.top = dimension.y;
                ...
                if (this.blocks[dimension.block_name] && dimension.block_unique_reference) {
                    ...
                    if (dimension_bbox.left < bbox.left) bbox.left = dimension_bbox.left;
                    if (dimension_bbox.right > bbox.right) bbox.right = dimension_bbox.right;
                    if (dimension_bbox.bottom < bbox.bottom) bbox.bottom = dimension_bbox.bottom;
                    if (dimension_bbox.top > bbox.top) bbox.top = dimension_bbox.top;
                }
            }
        }
    }
    ...
    if (depth === 0) {
        if (!isFinite(bbox.left)) bbox.left = 0;
        if (!isFinite(bbox.right)) bbox.right = 0;
        if (!isFinite(bbox.bottom)) bbox.bottom = 0;
        if (!isFinite(bbox.top)) bbox.top = 0;
    }

    // If we have appropriate values then calculate width and height.
    if (isFinite(bbox.left) && isFinite(bbox.right)) bbox.width = bbox.right - bbox.left;
    if (isFinite(bbox.top) && isFinite(bbox.bottom)) bbox.height = bbox.top - bbox.bottom;
    return bbox;
}
...
Collector.prototype.toSVG = function Collector_toSVG(layers, transform, compact, asgroup) {
    ...
    for (var i = 0; i < texts.length; i++) {
        ...
        if (allLayers || layers.indexOf(text.layer) !== -1) {
            ...
            output += '<text x="' + floatToString(text.x - transform.translate_x) + '" y="' + floatToString(-text.y - transform.translate_y) + '" font-size="1em" font-weight="' + (text.thickness || 0) + '"  transform="rotate(' + text.rotation + ' ' + pointToString(text.x - transform.translate_x, -text.y - transform.translate_y) + ')" text-anchor="' + anchor + '" alignment-baseline="' + valign + '">' + escapeHtml(text.text) + '</text>';
            if (!compact) output += nl;
        }
    }
