...
function getBBox(collector, layers, depth) {
  ...
  for (i = 0; i < lines.length; i++) {
    var line = lines[i];
    if (allLayers || layers.indexOf(line.layer) !== -1) {
      bbox.expand(line.start.x, line.start.y);
      bbox.expand(line.end.x, line.end.y);
    }
  }
  // Process Poly Line
  var polylines = collector.lwpolylines;
  ...
  for (i = 0; i < circles.length; i++) {
    var circle = circles[i];
    if (allLayers || layers.indexOf(circle.layer) !== -1) {
      bbox.expandByPoint(point.x - circle.r, point.y - circle.r);
      bbox.expandByPoint(point.x + circle.r, point.y + circle.r);
    }
  }
  // TODO BBox of Text
  ...
  for (i in blocks) {
    ...
    if (i.charAt(0) !== '*' && blocks.hasOwnProperty(i) && allLayers || layers.indexOf(i) !== -1) {
      var block_bbox = getBBox(blocks[i], '*', depth + 1);
    ...
    if (allLayers || layers.indexOf(dimension.layer) !== -1) {
      if (dimension.block_name) {
        ...
        if (collector.blocks[dimension.block_name] && dimension.block_unique_reference) {
          var dimension_bbox = collector.blocks[dimension.block_name].getBBox(layers, depth + 1);
