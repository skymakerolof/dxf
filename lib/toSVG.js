...
module.exports = function(parsed) {
  ...
  polylines.forEach((polyline, i) => {
    ...
    if (!layerTable) {
      logger.error('no layer table for layer:', entity.layer);
    }
    const layerColorNumber = (layerTable === undefined) ?
      [0, 0, 0] : layerTable.colorNumber;
    const rgb = colors[layerColorNumber];
