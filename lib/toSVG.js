...
function spline_toSVGPath(spline, transform) {
  ...
  var output = points.reduce(function(acc, point, i) {
    acc += (i === 0) ?'M' : 'L';
    acc += pointToString(point[0], point[1], transform);
    return acc;
  }, '');
  //
  // var output = '';
...
function ellipse_toSVGPath2(ellipse, transform) {
  ...
  // Start point
  points.push([
    Math.cos(ellipse.startAngleRad)*rx,
    Math.sin(ellipse.startAngleRad)*ry
  ]);
  // Between
  for (var i = 0; i <= 72; ++i) {
    var theta = i/72*Math.PI*2;
    if ((theta > ellipse.startAngleRad) && (theta < ellipse.endAngleRad)) {
  ...
  // End point
  points.push([
    Math.cos(ellipse.endAngleRad)*rx,
    Math.sin(ellipse.endAngleRad)*ry
  ]);

  // Rotate
  points = points.map(function(p) {
    return [
      p[0]*Math.cos(majorAxisRotation) - p[1]*Math.sin(majorAxisRotation),
      p[1]*Math.cos(majorAxisRotation) + p[0]*Math.sin(majorAxisRotation),
    ];
  });

  var output = points.reduce(function(acc, point, i) {
    acc += (i === 0) ?'M' : 'L';
    acc += pointToString(ellipse.x + point[0], ellipse.y + point[1], transform);
    return acc;
  }, '');
  return output;
}
...
function getBBox(collector, layers, depth) {
  ...
  for (i = 0; i < arcs.length; i++) {
    ...
    if (allLayers || layers.indexOf(arc.layer) !== -1) {
      ...
      bbox.expandByPoint(arc.x + arc.r, arc.y + arc.r);
    }
  }
  ...
  // Process Ellipses
  var ellipses = collector.ellipses;
  for (i = 0; i < ellipses.length; i++) {
    var ellipse = ellipses[i];
    // TODO: Use major/minor
    if (allLayers || layers.indexOf(ellipse.layer) !== -1) {
      var r = Math.sqrt(ellipse.majorX*ellipse.majorX + ellipse.majorY*ellipse.majorY);
      bbox.expandByPoint(ellipse.x - r, ellipse.y - r);
      bbox.expandByPoint(ellipse.x + r, ellipse.y + r);
    }
  }
  ...
  for (i = 0; i < dimensions.length; i++) {
    ...
    if (allLayers || layers.indexOf(dimension.layer) !== -1) {
      if (dimension.block_name) {
        bbox.expandByPoint(dimension.x, dimension.y);
        if (collector.blocks[dimension.block_name] && dimension.block_unique_reference) {
          ...
          bbox.expandByBox(dimension_bbox);
        }
      }
    }
  }
...
function toSVG(collector, layers, transform, asgroup) {
  ...
  if (asgroup === undefined) {
    ...
    output += '<defs>';
    output += '<marker markerWidth="15" markerHeight="15" orient="auto" ' +
      'refX="0" refY="0" id="longDimensionArrow">';
    output += '  <path d="m0,15 l30,5 l0,-10z" />';
    output += '</marker>';
    output += '<marker markerWidth="15" markerHeight="15" orient="auto" ' +
      'refX="0" refY="2.5" id="longDimensionArrowEnd">';
  ...
  for (i = 0; i < lines.length; i++) {
    ...
    if (allLayers || layers.indexOf(line.layer) !== -1) {
      ...
      output += '<line style="' + startMarker + endMarker +
        ...
        '" stroke="black" stroke-width="' + (line.thickness || 0.5) + '"></line>';
    }
  }
  // Process Poly Line
  var polylines = collector.lwpolylines;
  for (i = 0; i < polylines.length; i++) {
    var polyline = polylines[i];
    if (allLayers || layers.indexOf(polyline.layer) !== -1) {
      output += '<path fill="none" stroke="black" stroke-width="0.5" d="' +
  ...
  for (i = 0; i < ellipses.length; i++) {
    ...
    if (allLayers || layers.indexOf(ellipse.layer) !== -1) {
      output += '<path fill="none" stroke="black" stroke-width="0.5" d="' +
        ellipse_toSVGPath2(ellipse, transform) + '"/>';
      //
      // output += '<ellipse fill="none" stroke="black" stroke-width="0.5"' +
      //   ' cx="' + floatToString(ellipse.x - transform.translate_x) + '"' +
      //   ' cy="' + floatToString(-ellipse.y - transform.translate_y) + '"' +
      //   ' rx="' + floatToString(Math.abs(ellipse.majorX) * transform.scale) + '"' +
      //   ' ry="' + floatToString(Math.abs(ellipse.majorY) * transform.scale) + '"' +
      //   '/>';
    }
  }
  ...
  for (i = 0; i < circles.length; i++) {
    ...
    if (allLayers || layers.indexOf(circle.layer) !== -1) {
      ...
      output += '<circle cx="' + floatToString(circle.x - transform.translate_x) +
        ...
        '" stroke-width="' + (circle.thickness || 0.5) + '" />';
    }
  }
  // Process Hatches
  var hatches = collector.hatches;
  ...
  for (i = 0; i < texts.length; i++) {
    ...
    if (allLayers || layers.indexOf(text.layer) !== -1) {
      ...
      output += '<text x="' + floatToString(text.x - transform.translate_x) +
        ...
        '" font-family="Monospace" font-size="5" font-weight="' + (text.thickness || 0) +
  ...
  if (asgroup) {
    ...
    var svgString = '<?xml version="1.0"?><svg' + id + ' xmlns="http://www.w3.org/2000/svg"' +
    ...
    if (bbox) {
      ...
      svgString += ' viewBox="' +
        ...
        ' ' + floatToString(bbox.width + 2) +
        ' ' + floatToString(bbox.height + 2) + '"';
    }
